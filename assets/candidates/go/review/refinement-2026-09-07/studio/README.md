...
**Atualização do salto:** o [jump v2 foi recarregado e recapturado](jump-v2/README.md) em três quadros, substituindo a evidência do salto anterior. Os outros 19 clips / 57 quadros permanecem documentados pela primeira execução abaixo. O [texto original da primeira inspeção](README-before-jump-v2.md), os 60 PNGs e seu relatório foram preservados.

Na primeira execução, o manifesto canônico foi aberto pelo seletor GTK real e percorrido pelos botões de clip e `Next frame` via AT-SPI: **20 clips, 60 quadros, zoom 1.00x**. Cada captura é o framebuffer da janela nativa, obtido com ImageMagick `import -window 0xa00003`. O JSON exibido pela interface era idêntico ao candidato daquela execução; pivô X/Y, duração e escala dos 60 quadros foram comparados ao manifesto.
...
| [jump v2](jump-v2/jump-v2-native-panel.png) | 3 recapturados | Nova fonte com cabeça/tronco corrigidos e escala única 0,37; ver [recaptura](jump-v2/README.md). O [painel v1](panels/jump.png) é histórico superado. |
...
A nova identidade foi mantida: gopher adulto azul-ardósia, peito claro, focinho modelado, olhos pequenos, calça carvão, faixa preta, luvas sem dedos e patas nuas. A primeira inspeção não identificou recortes impeditivos nem troca de identidade. Uma auditoria independente posterior detectou o salto com cabeça/tronco reduzidos; a nova geração v2 e sua recaptura substituem essa parte da avaliação. O contorno claro na pelagem acompanha a luz pintada do master.
