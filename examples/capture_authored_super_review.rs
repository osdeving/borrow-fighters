...
//! --no-damage exercises the match flags; --barrage-log records every C++ beat.
...
use borrow_fighters::{
    ...
    combat::{
        ...
        super_sequence::{
            ...
            RUST_ARENA_COMMIT_TICK, SuperPhase, super_spec,
        },
    },
    ...
    engine::{
        ...
        sprites::{fighter_sprite_clip, frame_for_clip_at, frame_for_fighter_state},
    },
    game::{
        arena::ArenaId,
    ...
    fn parse() -> ReviewResult<Self> {
        ...
        while let Some(arg) = args.next() {
            match arg.as_str() {
                ...
                "--opponent" => result.opponent = Some(match args.next().ok_or("--opponent needs rust/duke/go/c/cpp/python")?.as_str() {
                    "rust" => CharacterId::Rust, "duke" | "java" => CharacterId::Duke, "go" => CharacterId::Go, "c" => CharacterId::C, "cpp" | "c++" => CharacterId::Cpp, "python" | "py" => CharacterId::Python,
                    _ => return Err("unknown opponent; use rust/duke/go/c/cpp/python".into()),
                }),
                ...
                "--no-damage" => result.no_damage = true,
                "--barrage-log" => result.barrage_log = true,
                _ => return Err(format!("unknown argument {arg}; use --output DIR [--character rust|duke|c|cpp|python] [--opponent rust|duke|go|c|cpp|python] [--arena ARENA] [--guard] [--no-damage] [--barrage-log] [--reverse|--both-sides] [--video PATH] [--audio] [--realtime] [--no-snapshots]").into()),
            }
        }
...
fn main() -> ReviewResult<()> {
    ...
    for character in characters {
        for &reverse in &sides {
            ...
            let opponent = options
                .opponent
                .unwrap_or(if character == CharacterId::Rust {
                    CharacterId::Duke
                } else {
                    CharacterId::Rust
                });
            let mut world = World::new_with_character_body_metrics(character, opponent, &metrics);
            let initial_health = [world.player_one.health, world.player_two.health];
            ...
            if character == CharacterId::Cpp {
                ...
                if options.barrage_log {
                    for hit in 0..CPP_BARRAGE_HITS {
                        for age in [0, 3, 7] {
                            schedule.insert(
                                CPP_BARRAGE_START + hit * CPP_BARRAGE_CADENCE + age,
                                format!("barrage-{}-age-{age}", hit + 1),
                            );
                        }
                    }
                }
            ...
            let mut barrage_frames = Vec::new();
            ...
            println!(
                "REVIEW_START {} opponent={} reverse={reverse} guard={} no_damage={} frame={global_frame}",
                character.audio_key(),
                opponent.audio_key(),
                options.guard,
                options.no_damage
            ...
            for frame in 0..PREPARATION + spec.duration_frames + RESTORED_OBSERVATION {
                ...
                if options.barrage_log
                    && let Some(sequence) = snapshot.filter(|sequence| {
                        sequence.character == CharacterId::Cpp
                            && (CPP_BARRAGE_START..CPP_FINISHER_TICK).contains(&sequence.tick)
                    })
                {
                    let beat = (sequence.tick - CPP_BARRAGE_START) / CPP_BARRAGE_CADENCE;
                    barrage_frames.push(json!({
                        "sequence_tick":sequence.tick,"video_frame":output_frame,
                        "barrage_hit":beat+1,
                        "contact_tick":CPP_BARRAGE_START+beat*CPP_BARRAGE_CADENCE,
                        "age_since_contact_frames":(sequence.tick-CPP_BARRAGE_START)%CPP_BARRAGE_CADENCE,
                        "health":[world.player_one.health,world.player_two.health],
                        "target":target_observation(&world, &assets, opponent)?,
                    }));
                }
                    ...
                    || options.realtime
                {
                    ...
                    if let Some(label) = scheduled.map(String::as_str).or(ordinary_snapshot) {
                        ...
                        screenshots.push(json!({"image":name,"effective_arena":format!("{effective_arena:?}"),"video_frame":output_frame,"simulation_frame":global_frame,"phase":snapshot.map(|sequence|format!("{:?}",sequence.phase())),"sequence_tick":snapshot.map(|sequence|sequence.tick),"health":[world.player_one.health,world.player_two.health],"actor_x":world.player_one.position.x,"target_x":world.player_two.position.x,"target":target_observation(&world, &assets, opponent)?}));
                    }
            ...
            if world.super_sequence_active()
                ...
                || (options.no_damage
                    && [world.player_one.health, world.player_two.health] != initial_health)
            {
                return Err(format!("incorrect final result for {character:?}/{reverse}: expected target damage {expected}, health {:?} -> {:?}", initial_health, [world.player_one.health, world.player_two.health]).into());
            }
            scenarios.push(json!({"character":character.audio_key(),"opponent":opponent.audio_key(),"move":spec.label,"arena":format!("{arena:?}"),"final_effective_arena":format!("{effective_arena:?}"),"arena_transitions":arena_transitions,"music_track":music_for_arena(arena).key(),"reverse":reverse,"guarded":options.guard,"no_damage":options.no_damage,"initial_health":initial_health,"final_health":[world.player_one.health,world.player_two.health],"duration_frames":spec.duration_frames,"simulation_frames":[initial_frame,global_frame-1],"video_frames":[initial_frame,global_frame-1],"wall_time_seconds":scenario_started.elapsed().as_secs_f64(),"damage":expected,"barrage_frames":barrage_frames,"screenshots":screenshots,"audio_events":cues}));
            println!(
    ...
    let manifest: Value = json!({"fps":60,"render_size":[WINDOW_WIDTH,WINDOW_HEIGHT],"audio":false,"live_audio_playback":options.audio,"realtime":options.realtime,"video":options.video,"video_frame_count":global_frame,"simulation_frame_count":global_frame,"snapshots":options.snapshots,"no_damage":options.no_damage,"barrage_log":options.barrage_log,"feature_flags":{"player_one_takes_damage":flags.enabled(FeatureFlag::PlayerOneTakesDamage),"player_two_takes_damage":flags.enabled(FeatureFlag::PlayerTwoTakesDamage)},"renderer":"draw_fight","render_wall_time_seconds":render_wall_seconds,"max_frame_lateness_seconds":max_frame_lateness_seconds,"video_duration_seconds":global_frame as f64/60.0,"audio_delivery":"silent deterministic video; optional offline mix from the real World cues recorded below","simulation":"World fixed-step with explicit feature flags; real contacts, physical C++ rush, snapshots at authored phase clocks","scenarios":scenarios});
    fs::write(
...
/// Records the target's contact clock and the frame used by normal match drawing.
/// The Python lunge is the only forced target pose during these live supers.
fn target_observation(
    ...
    let manifest = &atlas(assets, opponent)?.manifest;
    let state_clip = fighter_sprite_clip(target);
    let contact = target.contact_reaction_state();
    let forced_launch = world
        .super_sequence()
        .filter(|sequence| sequence.phase() == SuperPhase::PythonLunge);
    let frame = if let Some(sequence) = forced_launch {
        let time = (sequence.tick - sequence.phase_span().start) as f32 / 60.0;
        let clip = if target.uses_contact_reactions() {
            "reaction_launch"
        } else {
            "launched"
        };
        frame_for_clip_at(manifest, clip, time)
    } else {
        frame_for_fighter_state(manifest, target, state_clip, world.elapsed_seconds)
    };
    Ok(json!({
        "character":opponent.audio_key(),"health":target.health,
        "grounded":target.grounded,"reaction_visual_active":target.reaction_visual_state().is_some(),
        "hitstun":target.in_hitstun(),"blockstun":target.in_blockstun(),
        "air_reaction":target.in_air_reaction(),"knockdown":target.in_knockdown(),
        "hidden":world.super_sequence().is_some_and(|sequence|sequence.target_hidden()),
        "physical_x":target.position.x,"physical_y":target.position.y,
        "state_clip":state_clip.as_str(),
        "forced_clip":forced_launch.map(|_|if target.uses_contact_reactions() { "reaction_launch" } else { "launched" }),
        "sprite_frame":frame.map(|frame|frame.name.as_str()),
        "sprite_clip":frame.map(|frame|frame.clip.as_str()),
        "contact":contact.map(|contact|json!({
            "profile":format!("{:?}",contact.profile),
            "age_frames":(contact.elapsed_seconds*60.0).round() as u32,
            "duration_frames":contact.duration_seconds*60.0,
            "progress":contact.progress(),"cinematic":contact.cinematic,
        })),
    }))
}
