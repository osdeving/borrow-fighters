...
- origem de projectile calibrada no primeiro frame do clip `special` de Rust, Duke, Go, C, Python e C++;
...
- Go, C, Python e C++ ja possuem atlas placeholder; falta revisar boxes por frame e criterios de aceite visual.
- C++ usa manifest multi-atlas (`frames[].image`); a luta e o Combat Lab ja carregam esse formato, mas o viewer Raylib antigo deve ser considerado temporario ate o Sprite Studio cobrir esse fluxo.
