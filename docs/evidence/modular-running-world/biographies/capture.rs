...
use borrow_fighters::adventure::{
    engine::{assets::Assets, biography},
    text::TextCatalog,
};
use raylib::prelude::*;
use std::{
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let out = "docs/evidence/modular-running-world/biographies";
    for (id, tick, label) in [
        ("duke", 60, "duke-approaching"),
        ("duke", 175, "duke-parking"),
        ("duke", 345, "duke-arrival"),
    ...
    ] {
        {
            ...
            biography::draw(&mut d, &assets, id, tick);
        }
